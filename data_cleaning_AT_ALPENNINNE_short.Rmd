---
title: "Data cleaning AT_ALPENNINNE"
subtitle: "AT_ALPENNINNE"
...
samples <- read.csv("C:/Data/PAs/AT_ALPENNINNE.csv")
...
saveRDS(samples_organized, 
        file = here("r_objects", "AT_ALPENNINNE", "samples_organized.rds"))
```

### Save as csv

```{r}
water_samples_csv <- samples_organized %>% filter(label == 73) %>% ungroup() %>%
  select(-time_series)
write.csv(water_samples_csv, here("data", "clean", "water_samples.csv"))
```
...
saveRDS(samples_SOM_cluster_25,
        file = here("r_objects", "AT_ALPENNINNE", "samples_SOM_cluster_25.rds"))
```
...
saveRDS(samples_SOM_25_02_02_new_cluster,
        file = here("r_objects", "AT_ALPENNINNE",
...
samples_RSI <- sits_reduce_imbalance(
  ...
  n_samples_over = 179, # Min. number of samples per class
  # If doing like this, samples_RSI puts longitude and latitude = 0
  # for all samples in the class with minimum number of samples (label 52 here).
  # Not sure why this happens!
  # Try with n_samples_over = number of samples for the least abundant label - 1
  n_samples_under = 12470, # Max. number of samples per class (based on CLC 2018)
  multicores = 12
)
end_time <- Sys.time()
# Calculate the time difference in hours
time_samples_RSI <- difftime(end_time, start_time, units = "hours")
print(time_samples_RSI)
# 3.8 hours
saveRDS(samples_RSI, file = here("r_objects", "AT_ALPENNINNE", "samples_RSI.rds"))
```
...
som_cluster_bal <- sits_som_map(
  data = samples_RSI %>% select(-post_prob, -eval),
  grid_xdim = 25,
  grid_ydim = 25,
  alpha = 1.0,
  distance = "dtw",
  rlen = 20)
# Error: .check_samples: NA value not allowed for x - invalid samples - missing columns and/or data rows
end_time <- Sys.time()
# Calculate the time difference in hours
time_som_cluster_bal <- difftime(end_time, start_time, units = "hours")
print(time_som_cluster_bal)
saveRDS(som_cluster_bal, file = here("r_objects", "AT_ALPENNINNE", "som_cluster_bal.rds"))
```
...
saveRDS(cross_val, file = here("r_objects", "AT_ALPENNINNE", "cross_val.rds"))
# This accuracy does not guarantee a good classification result. 
...
saveRDS(cross_val_SOM,
        file = here("r_objects", "AT_ALPENNINNE", "cross_val_SOM.rds"))
```
...
saveRDS(cross_val_SOM_RSI,
        file = here("r_objects", "AT_ALPENNINNE", "cross_val_SOM_RSI.rds"))
```
