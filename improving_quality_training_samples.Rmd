...
Agglomerative Hierarchical Clustering (AHC). Higher computational complexity than SOM (so maybe SOM better for large data). AHC computes dissimilarity between two elements from a dataset. Decides which two clusters are merged at each iteration.

dist_method: specifies how to calculate the distance between two time series

linkage: defines the distance metric between clusters

To cut the dendrogram, the sits_cluster_dendro() function computes the adjusted rand index (ARI), returning the height where the cut of the dendrogram maximizes the index. The result of sits_cluster_dendro() is a time series tibble with one additional column called “cluster”. 

```{r eval=FALSE, include=TRUE}
# Realizamos la agrupación que hace HC automáticamente.
# Usamos un 1% de los datos y funciona
# Con 10% sigue dando error!
samples_HC_1 <- sits_cluster_dendro(samples = samples_organized_ungr %>%
                                    sample_frac(0.01),
                                  dist_method = "dtw_basic",
                                  linkage = "ward.D2")
saveRDS(samples_HC_1, file = here("r_objects", "samples_HC_1.rds"))
```

Try with one band only and 5% of the data (with all data still error).

```{r eval=FALSE, include=TRUE}
samples_HC_5_1band  <- sits_cluster_dendro(samples = samples_organized_ungr %>%
                                          sample_frac(0.05),
                                        bands = c("B02"),
                                        dist_method = "dtw_basic",
                                        linkage = "ward.D2")
saveRDS(samples_HC_5_1band , file = here("r_objects", "samples_HC_5_1band rds"))
```
...
The function sits_cluster_frequency() provides information on the composition of each cluster. Clusters might have a predominance of one label, or contain a mix of samples from several labels (confusion).

Use dplyr::filter() to remove very "mixed" clusters.

sits_cluster_clean() removes the outliers, leaving only the most frequent label for each cluster. After cleaning the samples, the resulting set of samples is likely to improve the classification results.

```{r eval=FALSE, include=TRUE}
sits_cluster_frequency(samples_HC_1)
sits_cluster_frequency(samples_HC_5_1band)
...
samples_HC1_clean <- sits_cluster_clean(samples_HC_1) # Leaves very few samples!
samples_HC5_1band_clean <- sits_cluster_clean(samples_HC_5_1band) # Leaves very few samples!
samples_HC1_clean
samples_HC5_1band_clean
```
